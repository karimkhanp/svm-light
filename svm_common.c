...
MODEL *read_model(char *modelfile)
{
    ...
    if (verbosity >= 1)
    {
        fprintf(stderr, "OK. (%d support vectors read)\n",
                (int) (model->sv_num - 1));
    }
    return (model);
}
...
MODEL *copy_model(MODEL *model)
{
    ...
    {
        newmodel->lin_weights = (double *) my_malloc(
                sizeof(double) * (model->totwords + 1));
        ...
        long int *totwords, long int *totdoc)
{
    ...
    read_documents(docfl, docs, label, totwords, totdoc);
        ...
        long int *totwords, long int *totdoc)
{
    ...
    while ((!feof(docfl)) && fgets(line, (int) ll, docfl))
    {
        ...
        /* fprintf(stderr,"docnum=%ld: Class=%f ",dnum,doc_label); */
        if (doc_label > 0)
            dpos++;
        if (doc_label < 0)
            dneg++;
    ...
    if (verbosity >= 1)
    {
        fprintf(stderr, "OK. (%ld examples read)\n", dnum);
    }
    (*totdoc) = dnum;
...
void nol_ll(FILE* fl, long int *nol, long int *wol, long int *ll)
{
    int ic;
    char c;
