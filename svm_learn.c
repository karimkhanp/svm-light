...
void svm_learn_classification(DOC **docs, double *cls, long int totdoc,
 ...
 optimization if not NULL. Array must be of size totdoc. */
{
    ...
    long heldout;
    long loo_count = 0, loo_count_pos = 0, loo_count_neg = 0, trainpos = 0,
            trainneg = 0;
    ...
    { /* default value for C */
        ...
        if (verbosity >= 1)
            fprintf(stderr, "Setting default regularization parameter C=%.4f\n",
                    learn_parm->svm_c);
    }
    ...
    if (alpha)
    {
        if (verbosity >= 1)
        {
            fprintf(stderr, "Computing starting state...");
            fflush(stderr);
        }
        ...
        for (i = 0; i < totdoc; i++)
        { /* create full index and clip alphas */
            index[i] = 1;
        ...
        update_linear_component(docs, label, index2dnum, alpha, a, index2dnum,
                                totdoc, totwords, kernel_parm, kernel_cache,
                                lin, aicache, weights);
    ...
    iterations = optimize_to_convergence(docs, label, totdoc, totwords,
                                         ...
                                         &shrink_state, model, inconsistent,
                                         unlabeled, a, lin, c, &timing_profile,
                                         &maxdiff, (long) -1, (long) 1);

    if (verbosity >= 1)
    {
        if (verbosity == 1)
            fprintf(stderr, "done. (%ld iterations)\n", iterations);
        ...
        fprintf(stderr,
                "Optimization finished (%ld misclassified, maxdiff=%.5f).\n",
        ...
        {
            fprintf(stderr,
                    "Runtime in cpu-seconds: %.2f (%.2f%% for kernel/%.2f%% for optimizer/%.2f%% for final/%.2f%% for update/%.2f%% for model/%.2f%% for check/%.2f%% for select)\n",
                    ((float) runtime_end - (float) runtime_start) / 100.0,
                    (100.0 * timing_profile.time_kernel)
                            / (float) (runtime_end - runtime_start),
                    (100.0 * timing_profile.time_opti)
                            / (float) (runtime_end - runtime_start),
                    (100.0 * timing_profile.time_shrink)
                            / (float) (runtime_end - runtime_start),
                    (100.0 * timing_profile.time_update)
                            / (float) (runtime_end - runtime_start),
                    (100.0 * timing_profile.time_model)
                            / (float) (runtime_end - runtime_start),
                    (100.0 * timing_profile.time_check)
                            / (float) (runtime_end - runtime_start),
                    (100.0 * timing_profile.time_select)
                            / (float) (runtime_end - runtime_start));
        }
        else
        {
            fprintf(stderr, "Runtime in cpu-seconds: %.2f\n",
        ...
        if (learn_parm->remove_inconsistent)
        {
            ...
            for (i = 0; i < totdoc; i++)
                if (inconsistent[i])
                    inconsistentnum++;
            fprintf(stderr,
                    "Number of SV: %ld (plus %ld inconsistent examples)\n",
        ...
        if ((verbosity >= 1) && (!learn_parm->skip_final_opt_check))
        {
            ...
            for (i = 0; i < totdoc; i++)
            {
                if ((lin[i] - model->b) * (double) label[i]
                        < 1.0 - learn_parm->epsilon_crit)
                    loss += 1.0 - (lin[i] - model->b) * (double) label[i];
                model_length += a[i] * label[i] * lin[i];
            }
            model_length = sqrt(model_length);
            fprintf(stderr, "L1 loss: loss=%.5f\n", loss);
            fprintf(stderr, "Norm of weight vector: |w|=%.5f\n", model_length);
            example_length = estimate_sphere(model, kernel_parm);
            fprintf(stderr,

                    "Norm of longest example vector: |x|=%.5f\n",
                    length_of_longest_document_vector(docs, totdoc,
                                                      kernel_parm));
            fprintf(stderr,

                    "Estimated VCdim of classifier: VCdim<=%.5f\n",
            ...
            if ((!learn_parm->remove_inconsistent) && (!transduction))
            {
                ...
                if (verbosity >= 1)
                {
                    fprintf(stderr, "Computing XiAlpha-estimates...");
                    fflush(stderr);
                }
                compute_xa_estimates(model, label, unlabeled, totdoc, docs, lin,
                                     a, kernel_parm, learn_parm,
                                     &(model->xa_error), &(model->xa_recall),
                                     &(model->xa_precision));
                if (verbosity >= 1)
                {
                    fprintf(stderr, "done\n");
                ...
                fprintf(stderr,
                        "Runtime for XiAlpha-estimates in cpu-seconds: %.2f\n",
                        (get_runtime() - runtime_start_xa) / 100.0);

                fprintf(stderr,

                        "XiAlpha-estimate of the error: error<=%.2f%% (rho=%.2f,depth=%ld)\n",
                        model->xa_error, learn_parm->rho, learn_parm->xa_depth);
                fprintf(stderr,

                        "XiAlpha-estimate of the recall: recall=>%.2f%% (rho=%.2f,depth=%ld)\n",
                        model->xa_recall, learn_parm->rho,
                        learn_parm->xa_depth);
                fprintf(stderr,

                        "XiAlpha-estimate of the precision: precision=>%.2f%% (rho=%.2f,depth=%ld)\n",
                        model->xa_precision, learn_parm->rho,
                        learn_parm->xa_depth);
            }
        ...
        if (verbosity >= 1)
        {
            fprintf(stderr, "Number of kernel evaluations: %ld\n",
    ...
    if (learn_parm->compute_loo)
    {
        ...
        if (verbosity >= 1)
        {
            fprintf(stderr, "Computing leave-one-out");
        }
        ...
        for (heldout = 0; (heldout < totdoc); heldout++)
        {
            ...
            else
            {
                ...
                if (((lin[heldout] - model->b) * (double) label[heldout])
                        <= 0.0)
                {
        ...
        optimize_to_convergence(docs, label, totdoc, totwords, learn_parm,
                                ...
                                inconsistent, unlabeled, a, lin, c,
        ...
        model->loo_recall = (1.0 - (double) loo_count_pos / (double) trainpos)
                * 100.0;
        ...
        {
            fprintf(stderr,
                    "Leave-one-out estimate of the error: error=%.2f%%\n",
                    model->loo_error);
            fprintf(stderr,
                    "Leave-one-out estimate of the recall: recall=%.2f%%\n",
                    model->loo_recall);
            fprintf(stderr,

            "Leave-one-out estimate of the precision: precision=%.2f%%\n",
                    model->loo_precision);
            fprintf(stderr, "Actual leave-one-outs computed:  %ld (rho=%.2f)\n",
                    loocomputed, learn_parm->rho);
            fprintf(stderr, "Runtime for leave-one-out in cpu-seconds: %.2f\n",
...
/* model:       Returns learning result (assumed empty before called) */
{
    ...
    {
        j = 2 * totdoc - 1 - i;
        docs[i] = create_example(i, 0, 0, docs_org[i]->costfactor,
                                 docs_org[i]->fvec);
        label[i] = +1;
        c[i] = value[i];
        docs[j] = create_example(j, 0, 0, docs_org[i]->costfactor,
                                 docs_org[i]->fvec);
        label[j] = -1;
        c[j] = value[i];
    }
    ...
    if (*kernel_cache)
    {
        kernel_cache_size = (*kernel_cache)->buffsize * sizeof(CFLOAT)
                / (1024 * 1024);
    ...
    {
        fprintf(stderr,
                "WARNING: Using a kernel cache for linear case will slow optimization down!\n");
    }
    ...
    if (verbosity >= 1)
    {
        ...
        fprintf(stderr, "Optimization finished (maxdiff=%.5f).\n", maxdiff);
        ...
        if ((verbosity >= 1) && (!learn_parm->skip_final_opt_check))
        {
            ...
            {
                if ((lin[i] - model->b) * (double) label[i]
                        < (-learn_parm->eps + (double) label[i] * c[i])
                                - learn_parm->epsilon_crit)
                    loss += -learn_parm->eps + (double) label[i] * c[i]
                            - (lin[i] - model->b) * (double) label[i];
                model_length += a[i] * label[i] * lin[i];
            }
            model_length = sqrt(model_length);
            fprintf(stderr, "L1 loss: loss=%.5f\n", loss);
...
void svm_learn_optimization(DOC **docs, double *rhs, long int totdoc,
                            long int totwords, LEARN_PARM *learn_parm,
                            KERNEL_PARM *kernel_parm,
                            KERNEL_CACHE *kernel_cache, MODEL *model,
                            double *alpha)
 ...
 optimization if not NULL. Array must be of size totdoc. */
{
    ...
    if (learn_parm->remove_inconsistent)
    {
        ...
        fprintf(stderr,
                "'remove inconsistent' not available in this mode. Switching option off!");
        fflush(stderr);
    }
    ...
    if ((verbosity >= 1) && (!learn_parm->skip_final_opt_check))
    {
        if (learn_parm->sharedslack)
        {
            fprintf(stderr, "Number of SV: %ld\n", model->sv_num - 1);
            fprintf(stderr,
                    "Number of non-zero slack variables: %ld (out of %ld)\n",
        ...
        example_length = estimate_sphere(model, kernel_parm);
        fprintf(stderr, "Norm of longest example vector: |x|=%.5f\n",
...
/* retrain: selects training mode (1=regular / 2=holdout) */
{
    ...
    qp.opt_ce = (double *) my_malloc(
            sizeof(double) * learn_parm->svm_maxqpsize);
    qp.opt_ce0 = (double *) my_malloc(sizeof(double));
    qp.opt_g = (double *) my_malloc(
            sizeof(double) * learn_parm->svm_maxqpsize
                    * learn_parm->svm_maxqpsize);
    qp.opt_g0 = (double *) my_malloc(
            sizeof(double) * learn_parm->svm_maxqpsize);
    qp.opt_xinit = (double *) my_malloc(
            sizeof(double) * learn_parm->svm_maxqpsize);
    qp.opt_low = (double *) my_malloc(
            sizeof(double) * learn_parm->svm_maxqpsize);
    qp.opt_up = (double *) my_malloc(
            sizeof(double) * learn_parm->svm_maxqpsize);
    weights = (double *) my_malloc(sizeof(double) * (totwords + 1));
    ...
    for (; retrain && (!terminate); iteration++)
    {
        ...
        if (verbosity >= 2)
        {
            fprintf(stderr, "Iteration %ld: ", iteration);
            fflush(stderr);
        }
        else if (verbosity == 1)
        {
        ...
        if (retrain == 2)
        {
            ...
            if (learn_parm->biased_hyperplane)
            {
                ...
                for (i = 0; (i < totdoc) && (fabs(eq) > learn_parm->epsilon_a);
                        i++)
                        {
        ...
        { /* select working set according to steepest gradient */
            if (iteration % 101)
            {
                ...
                choosenum += select_next_qp_subproblem_grad(
                        ...
                        selcrit, selexam, kernel_cache, 0, key, chosen);
            }
            else
        ...
        if (verbosity >= 2)
        {
            fprintf(stderr, " %ld vectors chosen\n", choosenum);
            fflush(stderr);
        }
        ...
        if (retrain != 2)
        {
            optimize_svm(docs, label, unlabeled, inconsistent, 0.0, chosen,
        ...
        update_linear_component(docs, label, active2dnum, a, a_old,
                                ...
                                kernel_cache, lin, aicache, weights);

        if (verbosity >= 2)
            t4 = get_runtime();
        supvecnum = calculate_svm_model(docs, label, unlabeled, lin, a, a_old,
                                        c, learn_parm, working2dnum,
                                        active2dnum, model);
        ...
        if (verbosity >= 3)
        {
            criterion = compute_objective_function(a, lin, c, learn_parm->eps,
                                                   label, active2dnum);
            fprintf(stderr,
                    "Objective function (over active variables): %.16f\n",
                    criterion);
            fflush(stderr);
        }
        ...
        retrain = check_optimality(model, label, unlabeled, a, lin, c, totdoc,
                                   learn_parm, maxdiff, epsilon_crit_org,
                                   &misclassified, inconsistent, active2dnum,
                                   last_suboptimal_at, iteration, kernel_parm);
                        ...
                        || (kernel_parm->kernel_type == LINEAR)))
        {
            ...
            t1 = get_runtime();
            reactivate_inactive_examples(label, unlabeled, a, shrink_state, lin,
                                         c, totdoc, totwords, iteration,
                                         learn_parm, inconsistent, docs,
                    ...
                    || (verbosity >= 2))
            {
                fprintf(stderr, "done.\n");
                fflush(stderr);
                fprintf(stderr, " Number of inactive variables = %ld\n",
                        inactivenum);
            }
        }
...
,                                            a,inconsistent);
	...
	kernel_cache_shrink(kernel_cache,totdoc,
        ...
        if ((!retrain) && learn_parm->remove_inconsistent)
        {
            ...
            {
                fprintf(stderr,
                        " Moving training errors to inconsistent examples...");
                fflush(stderr);
            }
            if (learn_parm->remove_inconsistent == 1)
            {
                retrain = identify_inconsistent(a, label, unlabeled, totdoc,
                                                learn_parm, &inconsistentnum,
                                                inconsistent);
            }
            else if (learn_parm->remove_inconsistent == 2)
            {
                retrain = identify_misclassified(lin, label, unlabeled, totdoc,
                                                 model, &inconsistentnum,
                                                 inconsistent);
            }
            ...
            if (verbosity >= 1)
            {
                ...
                if (retrain)
                {
                    fprintf(stderr, " Now %ld inconsistent examples.\n",
                            inconsistentnum);
                }
            }
        }
...
long optimize_to_convergence_sharedslack(DOC **docs, long int *label,
                                         ...
                                         KERNEL_CACHE *kernel_cache,
                                         SHRINK_STATE *shrink_state,
                                         MODEL *model, double *a, double *lin,
    ...
    for (; retrain && (!terminate); iteration++)
    {
        ...
        if (iteration % 101)
        {
            slackset = select_next_qp_slackset(docs, label, a, lin, slack,
                                               alphaslack, c, learn_parm,
                                               active2dnum, &maxsharedviol);
            if ((iteration % 2) || (!slackset)
...
;                            choosenum+=already_chosen;
                ...
                choosenum += select_next_qp_subproblem_grad(
                        ...
                        minl(learn_parm->svm_maxqpsize - choosenum,
                             learn_parm->svm_newvarsinqp - already_chosen),
                        learn_parm, inconsistent, active2dnum, working2dnum,
                        selcrit, selexam, kernel_cache, (long) 0, key, chosen);
            }
            else
            { /* do a step with all examples from same slack set */
                if (verbosity >= 2)
                {
                    fprintf(stderr, "(j-step on %ld)", slackset);
                    fflush(stderr);
                }
                jointstep = 1;
                for (jj = 0; (j = working2dnum[jj]) >= 0; jj++)
                ...
                { /* mask all but slackset */
                    ...
                    else
                    {
                        ...
                        /* fprintf(stderr,"Inslackset(%ld,%ld)",j,shrink_state->active[j]); */
                    }
                }
                ...
                choosenum = select_next_qp_subproblem_grad(
                        ...
                        kernel_cache, (long) 0, key, chosen);
                learn_parm->biased_hyperplane = 0;
            }
        }
        else
                        ...
                        || (kernel_parm->kernel_type == LINEAR)))
        {
            ...
            retrain = check_optimality_sharedslack(docs, model, label, a, lin,
                                                   c, slack, alphaslack, totdoc,
                                                   learn_parm, maxdiff,
                                                   epsilon_crit_org,
                                                   &misclassified, active2dnum,
                                                   last_suboptimal_at,
                                                   iteration, kernel_parm);
...
double compute_objective_function(double *a, double *lin, double *c, double eps,
                                  long int *label, long int *active2dnum)
...
/* Do optimization on the working set. */
{
    ...
    compute_matrices_for_optimization(docs, label, unlabeled,
                                      ...
                                      active2dnum, working2dnum, model, a, lin,
                                      c, varnum, totdoc, learn_parm, aicache,
                                      kernel_parm, qp);
...
void compute_matrices_for_optimization(DOC **docs, long int *label,
                                       long int *unlabeled,
                                       long *exclude_from_eq_const,
                                       double eq_target, long int *chosen,
                                       long int *active2dnum, long int *key,
                                       MODEL *model, double *a, double *lin,
                                       double *c, long int varnum,
                                       long int totdoc, LEARN_PARM *learn_parm,
                                       CFLOAT *aicache,
                                       KERNEL_PARM *kernel_parm, QP *qp)
{
    ...
    {
        fprintf(stderr,
                "Computing qp-matrices (type %ld kernel [degree %ld, rbf_gamma %f, coef_lin %f, coef_const %f])...",
                kernel_parm->kernel_type, kernel_parm->poly_degree,
                kernel_parm->rbf_gamma, kernel_parm->coef_lin,
    ...
    for (i = 0; i < varnum; i++)
    {
        ...
        for (j = i + 1; j < varnum; j++)
        {
            ...
            /* compute quadratic part of objective function */
            qp->opt_g[varnum * i + j] = (double) label[ki] * (double) label[kj]
                    * kernel_temp;
            qp->opt_g[varnum * j + i] = (double) label[ki] * (double) label[kj]
                    * kernel_temp;
        }
...
/* of alpha. */
{
    ...
    /* No alpha in the working set not at bounds, so b was not
     ...
     case. */
    if (learn_parm->biased_hyperplane && (!b_calculated)
    ...
    for (ii = 0; (i = active2dnum[ii]) >= 0; ii++)
    {
        ...
        if ((a[i] > learn_parm->epsilon_a) && (dist > target))
        {
            ...
            { /* largest violation */
                ...
                if (verbosity >= 5)
                    fprintf(stderr,
                            "sid %ld: dist=%.2f, target=%.2f, slack=%.2f, a=%f, alphaslack=%f\n",
                            docs[i]->slackid, dist, target,
                            slack[docs[i]->slackid], a[i],
                            alphaslack[docs[i]->slackid]);
                if (verbosity >= 5)
                    fprintf(stderr, " (single %f)\n", (*maxdiff));
            }
        }
...
long identify_inconsistent(double *a, long int *label, long int *unlabeled,
                           long int totdoc, LEARN_PARM *learn_parm,
                           long int *inconsistentnum, long int *inconsistent)
{
    ...
    for (i = 0; i < totdoc; i++)
    {
                ...
                && (a[i] >= (learn_parm->svm_cost[i] - learn_parm->epsilon_a)))
        {
            ...
            if (verbosity >= 3)
            {
                fprintf(stderr, "inconsistent(%ld)..", i);
                fflush(stderr);
            }
        }
    }
...
long identify_misclassified(double *lin, long int *label, long int *unlabeled,
...
void update_linear_component(DOC **docs, long int *label, long int *active2dnum,
                             double *a, double *a_old, long int *working2dnum,
                             long int totdoc, long int totwords,
                             KERNEL_PARM *kernel_parm,
                             KERNEL_CACHE *kernel_cache, double *lin,
                             CFLOAT *aicache, double *weights)
...
long incorporate_unlabeled_examples(MODEL *model, long int *label,
                                    long int *inconsistent, long int *unlabeled,
                                    double *a, double *lin, long int totdoc,
                                    double *selcrit, long int *select,
                                    long int *key, long int transductcycle,
                                    KERNEL_PARM *kernel_parm,
                                    LEARN_PARM *learn_parm)
{
    long i, j, k, j1, j2, j3, j4, unsupaddnum1 = 0, unsupaddnum2 = 0;
    long pos, neg, upos, uneg, orgpos, orgneg, nolabel, newpos, newneg,
            allunlab;
    ...
    for (i = 0; i < totdoc; i++)
    {
        ...
        if ((!unlabeled[i])
                && (a[i] > (learn_parm->svm_cost[i] - learn_parm->epsilon_a)))
        {
            /*      fprintf(stderr,"Ubounded %ld (class %ld, unlabeled %ld)\n",i,label[i],unlabeled[i]); */
        }
    }
    if (verbosity >= 2)
    {
        fprintf(stderr, "POS=%ld, ORGPOS=%ld, ORGNEG=%ld\n", pos, orgpos,
                orgneg);
        fprintf(stderr, "POS=%ld, NEWPOS=%ld, NEWNEG=%ld\n", pos, newpos,
                newneg);
        fprintf(stderr, "pos ratio = %f (%f).\n",
    ...
    if (transductcycle == 0)
    {
        ...
        if (verbosity >= 1)
        {
            /* fprintf(stderr,"costratio %f, costratio_unlab %f, unlabbound %f\n",
             learn_parm->svm_costratio,learn_parm->svm_costratio_unlab,
             learn_parm->svm_unlabbound); */
            fprintf(stderr,
                    "Classifying unlabeled data as %ld POS / %ld NEG.\n",
    ...
    if ((transductcycle % check_every) == 0)
    {
        ...
        if (verbosity >= 2)
        {
            /* fprintf(stderr,"costratio %f, costratio_unlab %f, unlabbound %f\n",
             ...
             learn_parm->svm_unlabbound); */
            fprintf(stderr,
                    "%ld positive -> Added %ld POS / %ld NEG unlabeled examples.\n",
    ...
    else if (((transductcycle % check_every) < check_every))
    {
        ...
        {
            fprintf(stderr,
                    "Model-length = %f (%f), loss = %f, objective = %f\n",
                    model_length, sumalpha, loss,
        ...
        if ((!unsupaddnum1) && (!unsupaddnum2))
        {
                    ...
                    && ((newpos + newneg) == allunlab))
            {
                ...
                write_prediction(learn_parm->predfile, model, lin, a, unlabeled,
                                 label, totdoc, learn_parm);
                if (verbosity >= 1)
                    fprintf(stderr, "Number of switches: %ld\n", switchnum);
                return ((long) 0);
            }
            ...
            if (verbosity >= 1)
                fprintf(stderr,
                        "Increasing influence of unlabeled examples to %f%% .",
                        learn_parm->svm_unlabbound * 100.0);
        }
        else if (verbosity >= 1)
        {
            fprintf(stderr,
                    "%ld positive -> Switching labels of %ld POS / %ld NEG unlabeled examples.",
                    upos, unsupaddnum1, unsupaddnum2);
            fflush(stderr);
        }
...
long select_next_qp_subproblem_grad(long int *label, long int *unlabeled,
                                    double *a, double *lin, double *c,
                                    long int totdoc, long int qp_size,
                                    LEARN_PARM *learn_parm,
                                    long int *inconsistent,
                                    long int *active2dnum,
                                    long int *working2dnum, double *selcrit,
                                    long int *select,
                                    KERNEL_CACHE *kernel_cache,
                                    long int cache_only, long int *key,
                                    long int *chosen)
 ...
 those for which the kernel evaluations are cached. */
{
    ...
    for (i = 0; (j = active2dnum[i]) >= 0; i++)
    {
        ...
        if (kernel_cache && cache_only)
            ...
            valid = 1;
        if (valid && (!((a[j] <= (0 + learn_parm->epsilon_a)) && (s < 0)))
                && (!((a[j] >= (learn_parm->svm_cost[j] - learn_parm->epsilon_a))
                        && (s > 0))) && (!chosen[j]) && (label[j])
                && (!inconsistent[j]))
        {
            selcrit[activedoc] = (double) label[j]
                    * (learn_parm->eps - (double) label[j] * c[j]
                            + (double) label[j] * lin[j]);
    ...
    for (k = 0;
            (choosenum < (qp_size / 2)) && (k < (qp_size / 2))
                    && (k < activedoc); k++)
            {
    ...
    select_top_n(selcrit, activedoc, select, (long) (qp_size / 2));
    for (k = 0; (choosenum < qp_size) && (k < (qp_size / 2)) && (k < activedoc);
            k++)
            {
        /* if(learn_parm->biased_hyperplane || (selcrit[select[k]] > 0)) { */
        i = key[select[k]];
...
long select_next_qp_subproblem_rand(long int *label, long int *unlabeled,
                                    ...
                                    long int *working2dnum, double *selcrit,
                                    long int *select,
                                    KERNEL_CACHE *kernel_cache, long int *key,
                                    long int *chosen, long int iteration)
/* Use the feasible direction approach to select the next
...
void select_top_n(double *selcrit, long int range, long int *select, long int n)
{
    register long i, j;
                       ...
                       long int maxhistory)
{
    ...
    shrink_state->a_history = (double **) my_malloc(
            sizeof(double *) * maxhistory);
...
long shrink_problem(DOC **docs, LEARN_PARM *learn_parm,
                    SHRINK_STATE *shrink_state, KERNEL_PARM *kernel_parm,
                    long int *active2dnum, long int *last_suboptimal_at,
...
void reactivate_inactive_examples(long int *label, long int *unlabeled,
                                  double *a, SHRINK_STATE *shrink_state,
                                  double *lin, double *c, long int totdoc,
                                  long int totwords, long int iteration,
                                  LEARN_PARM *learn_parm,
                                  long int *inconsistent, DOC **docs,
                                  KERNEL_PARM *kernel_parm,
                                  KERNEL_CACHE *kernel_cache, MODEL *model,
                                  CFLOAT *aicache, double *weights,
                                  double *maxdiff)
...
/* Computes lin for those variables from scratch. */
{
    ...
    { /* special linear case */
        ...
        for (i = 0; i < totdoc; i++)
        {
            ...
            {
                for (f = docs[i]->fvec; f; f = f->next)
                    lin[i] = shrink_state->last_lin[i]
                            + f->factor * sprod_ns(weights, f);
            }
    ...
    else
    {
        ...
        for (t = shrink_state->deactnum - 1;
                (t >= 0) && shrink_state->a_history[t]; t--)
                {
            if (verbosity >= 2)
            {
                fprintf(stderr, "%ld..", t);
            ...
            {
                get_kernel_row(kernel_cache, docs, i, totdoc, inactive2dnum,
                               aicache, kernel_parm);
                for (jj = 0; (j = inactive2dnum[jj]) >= 0; jj++)
                {
                    kernel_val = aicache[j];
                    lin[j] += (((a[i] * kernel_val) - (a_old[i] * kernel_val))
                            * (double) label[i]);
                }
            }
        }
    ...
    for (i = 0; i < totdoc; i++)
    {
        ...
        if (!inconsistent[i])
        {
            ...
            { /* make all active when sharedslack */
                shrink_state->active[i] = 1;
            }
        }
    }
    if (kernel_parm->kernel_type != LINEAR)
    { /* update history for non-linear */
        for (i = 0; i < totdoc; i++)
        {
            (shrink_state->a_history[shrink_state->deactnum - 1])[i] = a[i];
        }
        for (t = shrink_state->deactnum - 2;
                (t >= 0) && shrink_state->a_history[t]; t--)
                {
...
void get_kernel_row(KERNEL_CACHE *kernel_cache, DOC **docs, long int docnum,
                    long int totdoc, long int *active2dnum, CFLOAT *buffer,
                    KERNEL_PARM *kernel_parm)
...
void cache_kernel_row(KERNEL_CACHE *kernel_cache, DOC **docs, long int m,
                      KERNEL_PARM *kernel_parm)
 ...
 examples marked 0 in after. */
{
    ...
    if (verbosity >= 2)
    {
        fprintf(stderr, " Reorganizing cache...");
        fflush(stderr);
    }
    ...
    kernel_cache->max_elems = (long) (kernel_cache->buffsize
            / kernel_cache->activenum);
    ...
    if (verbosity >= 2)
    {
        fprintf(stderr, "done.\n");
        fflush(stderr);
        fprintf(stderr, " Cache-size in rows = %ld\n", kernel_cache->max_elems);
    }
}
...
KERNEL_CACHE *kernel_cache_init(long int totdoc, long int buffsize)
{
    ...
    kernel_cache->buffer = (CFLOAT *) my_malloc(
    ...
    {
        fprintf(stderr, " Cache-size in rows = %ld\n", kernel_cache->max_elems);
        fprintf(stderr, " Kernel evals so far: %ld\n", kernel_cache_statistic);
    }
...
void compute_xa_estimates(MODEL *model, long int *label, long int *unlabeled,
                          long int totdoc, DOC **docs, double *lin, double *a,
 ...
 Efficiently, IMCL, 2000. */
{
            ...
            if ((learn_parm->rho * a[i] * r_delta_sq + xi) >= 1.0)
            {
                ...
                { /* makes assumptions */
                    sim = distribute_alpha_t_greedily(
                            ...
                            kernel_parm,
                            learn_parm,
                            (double) ((1.0 - xi - a[i] * r_delta_sq)
                                    / (2.0 * a[i])));
                }
                if ((learn_parm->xa_depth == 0)
                        || ((a[i] * kernel(kernel_parm, docs[i], docs[i])
                                + a[i] * 2.0 * sim + xi) >= 1.0))
                {
                    looerror++;
                    if (label[i] > 0)
    ...
    (*precision) = (((double) totposex - (double) looposerror)
            / ((double) totposex - (double) looposerror + (double) loonegerror))
            * 100.0;
...
double distribute_alpha_t_greedily(long int *sv2dnum, long int svnum,
                                   DOC **docs, double *a, long int docnum,
                                   long int *label, KERNEL_PARM *kernel_parm,
 ...
 computing a better bound using a greedy optimzation strategy. */
{
        ...
        for (i = 0; i < svnum; i++)
        {
            ...
            if (!skip)
            {
                ...
                if (kernel_parm->kernel_type == LINEAR
                    ...
                    val += kernel(kernel_parm, docs[sv2dnum[i]],
                                  docs[sv2dnum[i]]);
                for (j = 0; j < d; j++)
                {
    ...
    /*  fprintf(stderr,"Distribute[%ld](%ld)=%f, ",docnum,best_depth,best); */
    return (best);
}
...
/* Loo-bound based on observation that loo-errors must have an
 ...
 Dissertation. */
{
    long i, j, l = 0, ulab = 0, lab = 0, labpos = 0, labneg = 0, ulabpos = 0,
            ulabneg = 0, totulab = 0;
    ...
    fprintf(stderr, "xacrit>=1: labeledpos=%.5f labeledneg=%.5f default=%.5f\n",
            (double) labpos / (double) totlab * 100.0,
            (double) labneg / (double) totlab * 100.0,
            (double) totlabpos / (double) (totlab) * 100.0);
    fprintf(stderr, "xacrit>=1: unlabelpos=%.5f unlabelneg=%.5f\n",
            (double) ulabpos / (double) totulab * 100.0,
            (double) ulabneg / (double) totulab * 100.0);
    fprintf(stderr, "xacrit>=1: labeled=%.5f unlabled=%.5f all=%.5f\n",
            (double) lab / (double) totlab * 100.0,
            (double) ulab / (double) totulab * 100.0,
            (double) l / (double) (totdoc) * 100.0);
    fprintf(stderr, "xacritsum: labeled=%.5f unlabled=%.5f all=%.5f\n",
            (double) labsum / (double) totlab * 100.0,
            (double) ulabsum / (double) totulab * 100.0,
            (double) (labsum + ulabsum) / (double) (totdoc) * 100.0);
    fprintf(stderr, "r_delta_sq=%.5f xisum=%.5f asum=%.5f\n", r_delta_sq, xisum,
            asum);
}
...
{ /* length of the longest support vector. This is */
    ...
    {
        xlen = sqrt(
                kernel(kernel_parm, model->supvec[j], model->supvec[j])
...
double estimate_r_delta(DOC **docs, long int totdoc, KERNEL_PARM *kernel_parm)
{
    ...
    {
        xlen = sqrt(
                kernel(kernel_parm, docs[i], docs[i])
...
void write_prediction(char *predfile, MODEL *model, double *lin, double *a,
                      long int *unlabeled, long int *label, long int totdoc,
                      LEARN_PARM *learn_parm)
{
    FILE *predfl;
    long i;
    ...
    if (verbosity >= 1)
    {
        fprintf(stderr, "Writing prediction file...");
        fflush(stderr);
    }
    ...
    for (i = 0; i < totdoc; i++)
    {
        if (unlabeled[i])
        {
            ...
            {
                dist = (double) label[i]
                        * (1.0 - learn_parm->epsilon_crit - a[i] / (a_max * 2.0));
            }
            else
            {
                dist = (lin[i] - model->b);
...
void write_alphas(char *alphafile, double *a, long int *label, long int totdoc)
{
    FILE *alphafl;
    long i;
    ...
    if (verbosity >= 1)
    {
        fprintf(stderr, "Writing alpha file...");
        fflush(stderr);
    }
