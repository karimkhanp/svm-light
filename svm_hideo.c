...
int optimize_hildreth_despo(long, long, double, double, double, long, long,
...
/* only semi-definite. */
{
    ...
    { /* allocate memory at first call */
        ...
        buffer = (double *) my_malloc(
                sizeof(double)
                        * ((nx + 1) * 2 * (nx + 1) * 2 + nx * nx
                                + 2 * (nx + 1) * 2 + 2 * nx + 1 + 2 * nx + nx
                                + nx + nx * nx));
        (*threshold) = 0;
        for (i = 0; i < nx; i++)
        {
    ...
    result = optimize_hildreth_despo(qp->opt_n, qp->opt_m, opt_precision,
                                     (*epsilon_crit), learn_parm->epsilon_a,
                                     maxiter,
                                     /* (long)PRIMAL_OPTIMAL, */
                                     (long) 0,
                                     (long) 0, lindep_sensitivity, qp->opt_g,
                                     qp->opt_g0, qp->opt_ce, qp->opt_ce0,
                                     qp->opt_low, qp->opt_up, primal,
    ...
    if ((result != PRIMAL_OPTIMAL) || (!(roundnumber % 31)) || (progress <= 0))
    {
        ...
        result = optimize_hildreth_despo(qp->opt_n, qp->opt_m, opt_precision,
                                         (*epsilon_crit), learn_parm->epsilon_a,
                                         (long) maxiter, (long) PRIMAL_OPTIMAL,
                                         (long) SMALLROUND, lindep_sensitivity,
                                         qp->opt_g, qp->opt_g0, qp->opt_ce,
                                         qp->opt_ce0, qp->opt_low, qp->opt_up,
                                         primal, qp->opt_xinit, dual,
                                         nonoptimal, buffer, &progress);
        if (verbosity >= 3)
        {
...
int optimize_hildreth_despo(long n, long m, double precision,
                            double epsilon_crit, double epsilon_a, long maxiter,
                            long goal, long smallround,
                            double lindep_sensitivity, double* g, double* g0,
                            double* ce, double* ce0, double* low, double* up,
                            double* primal, double* init, double* dual,
                            long* lin_dependent, double* buffer,
                            double* progress)
{
    ...
    dual_old = &(buffer[n + (n + m) * 2 * (n + m) * 2 + (n + m) * 2 + n + m
            + n * n]);
    low_new = &(buffer[n + (n + m) * 2 * (n + m) * 2 + (n + m) * 2 + n + m
            + n * n + (n + m) * 2]);
    up_new = &(buffer[n + (n + m) * 2 * (n + m) * 2 + (n + m) * 2 + n + m
            + n * n + (n + m) * 2 + n]);
    start = &(buffer[n + (n + m) * 2 * (n + m) * 2 + (n + m) * 2 + n + m + n * n
            + (n + m) * 2 + n + n]);
    g_new = &(buffer[n + (n + m) * 2 * (n + m) * 2 + (n + m) * 2 + n + m + n * n
            + (n + m) * 2 + n + n + n]);
    temp = &(buffer[n + (n + m) * 2 * (n + m) * 2 + (n + m) * 2 + n + m + n * n
            + (n + m) * 2 + n + n + n + n * n]);

    b1 = -1;
    b2 = -1;
    for (i = 0; i < n; i++)
    { /* get variables with steepest feasible descent */
        sum = g0[i];
        for (j = 0; j < n; j++)
            sum += init[j] * g[i * n + j];
        sum = sum * ce[i];
    ...
    if ((b1 != b2) && (m == 1))
    {
                ...
                && (g[b1 * n + b2] == g[b2 * n + b2]))
        {
            ...
            { /* (ce[b1] != ce[b2]) */
                /* printf("case +!\n"); */
                t = ((ce[b2] / ce[b1]) * g0[b1] - g0[b2]
                        + ce0[0]
                                * (g[b1 * n + b1] * ce[b2] / ce[b1]
                                        - g[b1 * n + b2] / ce[b1]))
                        / ((ce[b2] * ce[b2] / (ce[b1] * ce[b1]))
                                * g[b1 * n + b1] + g[b2 * n + b2]
                                - 2 * (g[b1 * n + b2] * ce[b2] / ce[b1]))
                        - init[b2];
                changed = 1;
                if ((up[b2] - init[b2]) < t)
                {
    ...
    else if ((m > 0) && (g[b1 * n + b2] != 0 && g[b2 * n + b2] != 0)
            && (fabs(
                    g[b1 * n + b1] / g[b1 * n + b2]
                            - g[b1 * n + b2] / g[b2 * n + b2])
                    < lindep_sensitivity))
    ...
    if (n > 2)
    { /* switch, so that variables are better mixed */
    ...
    if (smallround == SMALLROUND)
    {
        ...
        if (m > 0)
        { /* for biased hyperplane, pick two variables */
            lin_dependent[0] = 0;
            lin_dependent[1] = 0;
        }
        else
        { /* for unbiased hyperplane, pick only one variable */
    ...
    {
        result = solve_dual(n_indep, m, precision, epsilon_crit, maxiter, g_new,
                            g0_new, ce_new, ce0_new, low_new, up_new, primal, d,
    ...
    if (verbosity >= 3)
    {
        printf("before(%.30f)...after(%.30f)...result_sd(%d)...", obj_before,
               obj_after, result);
    }
...
int solve_dual(long n, long m, double precision, double epsilon_crit,
               long maxiter, double* g, double* g0, double* ce, double* ce0,
               double* low, double* up, double* primal, double* d, double* d0,
               double* ig, double* dual, double* dual_old, double* temp,
               long goal)
...
/* equality constraints. */
{
    ...
    {
        for (j = 0; j < n; j++)
        { /* dual hessian for box constraints */
    ...
    for (i = 0; i < n; i++)
    { /* dual linear component for the box constraints */
        w = 0;
        for (j = 0; j < n; j++)
        {
            w += (ig[i * n + j] * g0[j]);
    ...
    while ((retrain) && (maxviol > 0) && (iter < (scalemaxiter * maxfaktor)))
    {
        ...
        if (m > 0)
        {
            for (i = 0; i < n; i++)
            {
                temp[i] = dual[i] - dual[i + n]
                        + ce[i] * (dual[n + n] - dual[n + n + 1]) + g0[i];
            }
        }
        else
        {
...
void linvert_matrix(double *matrix, long depth, double *inverse,
                    double lindep_sensitivity, long *lin_dependent) /* indicates the active parts of matrix on
 input and output*/
{
    long i, j, k;
    double factor;
    ...
    {
        if (lin_dependent[i]
                || (fabs(matrix[i * depth + i]) < lindep_sensitivity))
        {
            lin_dependent[i] = 1;
        }
        else
